  ...
  Widget build(BuildContext context) {
    ...
    if (title != null) {
      final EdgeInsets defaultTitlePadding = EdgeInsets.only(
        top: 24.0,
        left: icon == null ? 24.0 : 0.0,
        bottom: 24.0,
        right: content == null ? 20.0 : 0.0,
      );
      ...
      titleWidget = Padding(
        padding: EdgeInsets.only(
          ...
          left: icon == null
              ? effectiveTitlePadding.left * paddingScaleFactor
              : effectiveTitlePadding.left,
    ...
    if (content != null) {
      final EdgeInsets defaultContentPadding = EdgeInsets.only(
        top: 24.0,
        left: theme.useMaterial3 ? 16.0 : 20.0,
        right: 24.0,
        bottom: 24.0,
      );
      ...
      contentWidget = Padding(
        padding: EdgeInsets.only(
          ...
          left: title == null && icon == null
              ? effectiveContentPadding.left * paddingScaleFactor
    ...
    if (actions != null) {
      final double spacing = (buttonPadding?.vertical ?? 16) / 2;
...
///  * [showDialog], which actually displays the dialog and returns its result.
///  * <https://material.io/design/components/dialogs.html#simple-dialog>
class MSimpleDialog extends StatelessWidget {
  ...
  const MSimpleDialog({
    super.key,
    this.title,
    this.titlePadding = const EdgeInsets.fromLTRB(24.0, 24.0, 0, 24.0),
    this.titleTextStyle,
    this.children,
    this.contentPadding = const EdgeInsets.fromLTRB(12.0, 0.0, 16.0, 0.0),
    this.backgroundColor,
    this.elevation,
    this.shadowColor,
    this.surfaceTintColor,
    ...
    if (title != null) {
      ...
      titleWidget = Padding(
        padding: EdgeInsets.only(
          ...
          top: effectiveTitlePadding.top * paddingScaleFactor,
          right: children == null
              ? effectiveTitlePadding.right * paddingScaleFactor
              : effectiveTitlePadding.right,
        ),
        child: DefaultTextStyle(
...
Future<T?> showMAdaptiveDialog<T>({
  required BuildContext context,
  required WidgetBuilder builder,
...
///  * [DisplayFeatureSubScreen], which documents the specifics of how
///    [DisplayFeature]s can split the screen into sub-screens.
class MDialogRoute<T> extends RawDialogRoute<T> {
  /// A dialog route with Material entrance and exit animations,
  /// modal barrier color, and modal barrier behavior (dialog is dismissible
  /// with a tap on the barrier).
  MDialogRoute({
